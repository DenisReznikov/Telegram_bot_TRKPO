...
def do_start_weather(update: Update, context):
    update.message.reply_text(
        text="Введите название города или отправьте геолокацию."
    )
    return s.LOC_WEATHER
...
def do_city(update: Update, context):
    ...
    if answer == "Что-то пошло не так":
        update.message.reply_text(text="Я не знаю такого города. Введите название города заново")
        return s.LOC_WEATHER
    update.message.reply_text(text=answer)
    update.message.reply_text(text="Хотите ли вы посмотреть более подробную погоду в " + context.user_data.get('city') + ".",
                              reply_markup=get_yes_keyboard())
    return s.RESULT_WEATHER
